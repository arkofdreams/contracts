...
async function deploy(aod) {
  ...
  console.log(
    'Arkonians contract deployed to (update .env):',
    contract.address
  );
...
async function grant(role, aod, contract, provider) {
  ...
  const tx = await aod.grantRole(getRole(role), contract.address, {
    gasPrice,
    gasLimit
  });
...
async function main() {
  ...
  const contract = await deploy(network.contracts.token);
