...
// $ npx hardhat run scripts/AODToken/deployPrivateSale.js
...
async function grant(role, aod, sale, provider) {
  ...
  const tx = await aod.grantRole(getRole(role), sale.address, {
    gasPrice,
    gasLimit
  });
...
async function main() {
  ...
  const sale = await deploy(
    network.contracts.token, //aod
    '0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56', //busd
    network.contracts.multisig //fund
  );
  ...
  const token = await Token.attach(network.contracts.token);
