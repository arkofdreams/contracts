...
describe('ArkoniaSale Tests', function () {
  ...
  it('Should error when buying', async function () {
    ...
    await expect(
      owner.withVesting.buy(investor1.address, ethers.utils.parseEther('100000'), { value: ethers.utils.parseEther('1') })
    ).to.be.revertedWith('InvalidVesting()');
  });

  it('Should set the vesting stage', async function () {
    const { owner } = this.signers;
    await owner.withVesting.stage(ethers.utils.parseEther('0.00001'), ethers.utils.parseEther('1000000'));
    expect(await owner.withVesting.currentTokenPrice()).to.equal(ethers.utils.parseEther('0.00001'));
    expect(await owner.withVesting.currentTokenLimit()).to.equal(ethers.utils.parseEther('1000000'));

    await expect(
      owner.withVesting.stage(ethers.utils.parseEther('0.000005'), ethers.utils.parseEther('1000001'))
    ).to.be.revertedWith('InvalidStage()');

    await expect(
      owner.withVesting.stage(ethers.utils.parseEther('0.00002'), ethers.utils.parseEther('900000'))
    ).to.be.revertedWith('InvalidStage()');
  });

  it('Should buy', async function () {
    const { owner, investor1, investor2, investor3 } = this.signers;
    await owner.withVesting.buy(investor1.address, ethers.utils.parseEther('100000'), {
      value: ethers.utils.parseEther('1')
    });
    expect(await owner.withVesting.vestingTokens(investor1.address)).to.equal(ethers.utils.parseEther('100000'));
    expect(await owner.withVesting.etherCollected(investor1.address)).to.equal(ethers.utils.parseEther('1'));

    await owner.withVesting.buy(investor2.address, ethers.utils.parseEther('100000'), {
      value: ethers.utils.parseEther('1')
    });
    expect(await owner.withVesting.vestingTokens(investor2.address)).to.equal(ethers.utils.parseEther('100000'));
    expect(await owner.withVesting.etherCollected(investor2.address)).to.equal(ethers.utils.parseEther('1'));

    await owner.withVesting.buy(investor3.address, ethers.utils.parseEther('100000'), {
      value: ethers.utils.parseEther('1')
    });
    expect(await owner.withVesting.vestingTokens(investor3.address)).to.equal(ethers.utils.parseEther('100000'));
    expect(await owner.withVesting.etherCollected(investor3.address)).to.equal(ethers.utils.parseEther('1'));
  });

  it('Should vest', async function () {
    const { owner, investor3, investor4 } = this.signers;

    await owner.withVesting.vest(investor3.address, ethers.utils.parseEther('100000'));
    expect(await owner.withVesting.vestingTokens(investor3.address)).to.equal(ethers.utils.parseEther('200000'));
    expect(await owner.withVesting.etherCollected(investor3.address)).to.equal(ethers.utils.parseEther('1'));

    await owner.withVesting.vest(investor4.address, ethers.utils.parseEther('100000'));
    expect(await owner.withVesting.vestingTokens(investor4.address)).to.equal(ethers.utils.parseEther('100000'));
  ...
  it('Should not refund', async function () {
    ...
    await expect(owner.withVesting.refund(investor1.address)).to.be.revertedWith('InvalidRefund()');

    await expect(owner.withVesting.refund(investor2.address)).to.be.revertedWith('InvalidRefund()');

    await expect(owner.withVesting.refund(investor3.address)).to.be.revertedWith('InvalidRefund()');

    await expect(owner.withVesting.refund(investor4.address)).to.be.revertedWith('InvalidRefund()');
  });
  ...
  it('Should refund all', async function () {
    ...
    await expect(owner.withVesting.vest(investor1.address, ethers.utils.parseEther('100000'))).to.be.revertedWith(
      'InvalidVesting()'
    );
  ...
  it('Should unlock', async function () {
    ...
    const earned1 = ethers.utils
      .parseEther('100000')
      .mul(1000)
      .div(vested - (now - 1000));
    const earned2 = ethers.utils
      .parseEther('200000')
      .mul(1000)
      .div(vested - (now - 1000));
  ...
  it('Should withdraw', async function () {
    ...
    expect(await owner.withTreasury.provider.getBalance(owner.withTreasury.address)).to.equal(ethers.utils.parseEther('1'));

    expect(await owner.withEconomy.provider.getBalance(owner.withEconomy.address)).to.equal(ethers.utils.parseEther('1'));
  });
  ...
  it('Should release all', async function () {
    ...
    expect(await owner.withToken.balanceOf(investor2.address)).to.equal(ethers.utils.parseEther('100000'));

    expect(await owner.withToken.balanceOf(investor3.address)).to.equal(ethers.utils.parseEther('200000'));

    expect(await owner.withToken.balanceOf(investor4.address)).to.equal(ethers.utils.parseEther('100000'));
  });
});
