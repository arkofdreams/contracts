...
async function getSigners(name, ...params) {
  //deploy the contract
  const contract = await deployProxy(name, ...params);
...
describe('Treasury Tests', function () {
  before(async function () {
    const [
      ...
      fund2,
      fund3
    ] = await getSigners('Treasury');

    await owner.withContract.grantRole(
  ...
  it('Should return the correct tiers', async function () {
    ...
    expect(
      await requester.withContract.tierInfo(ethers.utils.parseEther('0.01'))
    ).to.equal(1);
    expect(
      await requester.withContract.tierInfo(ethers.utils.parseEther('0.05'))
    ).to.equal(1);

    expect(
      await requester.withContract.tierInfo(ethers.utils.parseEther('0.1'))
    ).to.equal(2);
    expect(
      await requester.withContract.tierInfo(ethers.utils.parseEther('0.5'))
    ).to.equal(2);

    expect(
      await requester.withContract.tierInfo(ethers.utils.parseEther('1'))
    ).to.equal(3);
    expect(
      await requester.withContract.tierInfo(ethers.utils.parseEther('5'))
    ).to.equal(3);

    expect(
      await requester.withContract.tierInfo(ethers.utils.parseEther('10'))
    ).to.equal(4);
    expect(
      await requester.withContract.tierInfo(ethers.utils.parseEther('20'))
    ).to.equal(4);

    expect(
      await requester.withContract.tierInfo(ethers.utils.parseEther('21'))
    ).to.equal(0);
  });
  ...
  it('Should request tx', async function () {
    const { requester, receiver } = this.signers;
    await expect(
      requester.withContract.request(
        1,
        receiver.address,
  ...
  it('Should error approving non existent request', async function () {
    ...
    await expect(approver1.withContract.approve(10)).to.be.revertedWith(
      'InvalidCall()'
    );
  });
  ...
  it('Should not allow duplicate approving', async function () {
    const { approver1 } = this.signers;
    await expect(approver1.withContract.approve(1)).to.be.revertedWith(
      'InvalidCall()'
    );
  });
  ...
  it('Should withdraw', async function () {
    ...
    const balance = parseFloat(
      ethers.utils.formatEther(await receiver.getBalance())
    );
    ...
    expect(
      parseFloat(ethers.utils.formatEther(await receiver.getBalance())) -
        balance
    ).to.be.above(0.049);
  ...
  it('Should not allow approving already withdrawn tx', async function () {
    ...
    await expect(approver4.withContract.approve(1)).to.be.revertedWith(
      'InvalidCall()'
    );
  });
  ...
  it('Should be able to cancel request', async function () {
    ...
    await requester.withContract.request(
      3,
      receiver.address,
      ethers.utils.parseEther('0.5'),
      this.txURI
  ...
  it('Should error when withdrawing the available funds in the contract', async function () {
    ...
    await expect(owner.withContract.withdraw(4)).to.be.revertedWith(
      'InvalidCall()'
    );
  });
});
