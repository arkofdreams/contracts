...
async function getSigners() {
  ...
  for (let i = 0; i < signers.length; i++) {
    ...
    const priSaleFactory = await ethers.getContractFactory('AODPrivateSale', signers[i])
    const preSaleFactory = await ethers.getContractFactory('AODPresale', signers[i])
...
describe('AODToken Tests', function () {
  ...
  it('Should return zero tokens vested', async function () {
    const { investor1 } = this.signers
    expect(
      parseInt(await investor1.priSale.totalVestedAmount(investor1.address, 1713632400))
    ).to.equal(0)
    expect(
      parseInt(await investor1.priSale.totalReleasableAmount(investor1.address, 1713632400))
    ).to.equal(0)
  })
  ...
  it('Should trigger token generting event', async function () {
    ...
    this.privateUnlockDate = this.tokenGeneratedEvent + parseInt(await owner.priSale.lockPeriod())
    ...
    this.presaleUnlockDate = this.tokenGeneratedEvent + parseInt(await owner.preSale.lockPeriod())
  ...
  it('Should calculate the correct vested amounts for private', async function () {
    ...
    expect(await investor1.priSale.totalVestedAmount(investor1.address, stages.pri.start)).to.equal(0)
    expect(await investor1.priSale.totalReleasableAmount(investor1.address, stages.pri.start)).to.equal(0)
    
    //at the tge amount should be zero
    expect(await investor1.priSale.totalVestedAmount(investor1.address, this.tokenGeneratedEvent)).to.equal(0)
    expect(await investor1.priSale.totalReleasableAmount(investor1.address, this.tokenGeneratedEvent)).to.equal(0)
    
    //at the tge+locked period amount should be the locked tokens
    expect(await investor1.priSale.totalVestedAmount(investor1.address, this.privateUnlockDate)).to.equal(0)
    expect(await investor1.priSale.totalReleasableAmount(investor1.address, this.privateUnlockDate)).to.equal(investor1.vestment.lockedTokens)

    //at one second after the locked period, amount should be fracitonal
    expect(await investor1.priSale.totalVestedAmount(investor1.address, this.privateUnlockDate + 1)).to.equal(investor1.vestment.tick)
    expect(await investor1.priSale.totalReleasableAmount(investor1.address, this.privateUnlockDate + 1)).to.equal(investor1.vestment.lockedTokens.add(investor1.vestment.tick))

    //at the end of the vesting period, amount should be all
    expect(await investor1.priSale.totalVestedAmount(investor1.address, stages.pri.vested)).to.equal(investor1.vestment.vestingTokens)
    expect(await investor1.priSale.totalReleasableAmount(investor1.address, stages.pri.vested)).to.equal(investor1.vestment.lockedTokens.add(investor1.vestment.vestingTokens))

    //at the end of the vesting period, amount should still be all
    expect(await investor1.priSale.totalVestedAmount(investor1.address, stages.pri.vested + 100)).to.equal(investor1.vestment.vestingTokens)
    expect(await investor1.priSale.totalReleasableAmount(investor1.address, stages.pri.vested + 100)).to.equal(investor1.vestment.lockedTokens.add(investor1.vestment.vestingTokens))
  })
  ...
  it('Should calculate the correct vested amounts for presale', async function () {
    ...
    //at the start amount should be zero
    expect(await investor5.preSale.totalVestedAmount(investor5.address, stages.pre.start)).to.equal(0)
    expect(await investor5.preSale.totalReleasableAmount(investor5.address, stages.pre.start)).to.equal(0)
    
    //at the tge amount should be zero
    expect(await investor5.preSale.totalVestedAmount(investor5.address, this.tokenGeneratedEvent)).to.equal(0)
    expect(await investor5.preSale.totalReleasableAmount(investor5.address, this.tokenGeneratedEvent)).to.equal(0)
    
    //at the tge+locked period amount should be the locked tokens
    expect(await investor5.preSale.totalVestedAmount(investor5.address, this.presaleUnlockDate)).to.equal(0)
    expect(await investor5.preSale.totalReleasableAmount(investor5.address, this.presaleUnlockDate)).to.equal(investor5.vestment.lockedTokens)

    //at one second after the locked period, amount should be fracitonal
    expect(await investor5.preSale.totalVestedAmount(investor5.address, this.presaleUnlockDate + 1)).to.equal(investor5.vestment.tick)
    expect(await investor5.preSale.totalReleasableAmount(investor5.address, this.presaleUnlockDate + 1)).to.equal(investor5.vestment.lockedTokens.add(investor5.vestment.tick))

    //at the end of the vesting period, amount should be all
    expect(await investor5.preSale.totalVestedAmount(investor5.address, stages.pre.vested)).to.equal(investor5.vestment.vestingTokens)
    expect(await investor5.preSale.totalReleasableAmount(investor5.address, stages.pre.vested)).to.equal(investor5.vestment.lockedTokens.add(investor5.vestment.vestingTokens))

    //at the end of the vesting period, amount should still be all
    expect(await investor5.preSale.totalVestedAmount(investor5.address, stages.pre.vested + 100)).to.equal(investor5.vestment.vestingTokens)
    expect(await investor5.preSale.totalReleasableAmount(investor5.address, stages.pre.vested + 100)).to.equal(investor5.vestment.lockedTokens.add(investor5.vestment.vestingTokens))
  })
