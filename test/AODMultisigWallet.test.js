...
async function getSigners() {
  ...
  const wallet = await deploy('AODMultisigWallet', busd.address)

  //attach contracts
  for (let i = 0; i < signers.length; i++) {
    const walletFactory = await ethers.getContractFactory('AODMultisigWallet', signers[i])
    const busdFactory = await ethers.getContractFactory('BUSDToken', signers[i])
...
describe('AODMultisig Tests', function () {
  before(async function() {
    ...
    await owner.busd.mint(owner.wallet.address, ethers.utils.parseEther('1000000'))
  ...
  it('Should request tx', async function() {
    ...
    await requester.wallet.request(1, receiver.address, ethers.utils.parseEther('100'))
  ...
  it('Should error when using the same tx id', async function() {
    const { requester, receiver } = this.signers
    await expect(
      requester.wallet.request(1, receiver.address, ethers.utils.parseEther('100'))
  ...
  it('Should request tx, have 1 approved and transferred', async function() {
    ...
    await approver1.wallet.request(2, receiver.address, ethers.utils.parseEther('100'))
  ...
  it('Should not allow approving already executed tx', async function() {
    ...
    await expect(approver4.wallet.approve(1)).to.be.revertedWith('Transaction already executed')
  })

  it('Should not allow duplicate approving', async function() {
    const { approver1, receiver } = this.signers
    await approver1.wallet.request(3, receiver.address, ethers.utils.parseEther('100'))
    const tx = await receiver.wallet.txs(3)
    expect(tx.beneficiary).to.equal(receiver.address)
    expect(tx.amount).to.equal(ethers.utils.parseEther('100'))
    expect(tx.approvals).to.equal(1)
    
    await expect(approver1.wallet.approve(3)).to.be.revertedWith('Already approved')
  })
})