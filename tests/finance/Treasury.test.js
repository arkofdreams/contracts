...
describe('Treasury Tests', function () {
  before(async function () {
    ...
    const [owner, requester, receiver, approver1, approver2, approver3, approver4, fund1, fund2, fund3] = await getSigners(
      'Treasury',
      signers[0].address
    );
  ...
  it('Should error when using the same tx id', async function () {
    const { requester, receiver } = this.signers;
    await expect(requester.withContract.request(1, receiver.address, ethers.utils.parseEther('0.5'))).to.be.revertedWith(
      'InvalidCall()'
    );
  });
  ...
  it('Should not allow less than minimum request', async function () {
    const { requester, receiver } = this.signers;
    await expect(requester.withContract.request(3, receiver.address, ethers.utils.parseEther('0.009'))).to.be.revertedWith(
      'InvalidCall()'
    );
  });
  ...
  it('Should error when requesting in a cooldown', async function () {
    const { requester, receiver } = this.signers;
    await expect(requester.withContract.request(3, receiver.address, ethers.utils.parseEther('0.05'))).to.be.revertedWith(
      'InvalidCall()'
    );
  });
  ...
  it('Should error approving non existent request', async function () {
    ...
    await expect(approver1.withContract.approve(10)).to.be.revertedWith('InvalidCall()');
  });
  ...
  it('Should not allow requests passed the available funds in the contract', async function () {
    const { requester, receiver } = this.signers;
    await expect(requester.withContract.request(3, receiver.address, ethers.utils.parseEther('20'))).to.be.revertedWith(
      'InvalidCall()'
    );
  });
